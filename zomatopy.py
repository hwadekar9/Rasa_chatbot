...
class Zomato:
    ...
    def restaurant_search(self, query="", latitude="", longitude="", cuisines="", sort_by="rating", order="desc", start=0, limit=5):
        """
        ...
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?q=" + str(query) + "&start=" + str(start) + "&count=" + str(limit) + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines) + "&sort=" +str(sort_by) + "&order=" + str(order), headers=headers).content).decode("utf-8")
